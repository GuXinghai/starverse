...
describe('epoch-2 config projection network policy', () => {
  it('preserves an already valid V2 proxy settings object exactly', async () => {
    ...
    const projected = await projectEpoch2Config({ rawConfig: { networkProxySettingsV2: settings } })
  ...
  it('converts only a recognized legacy system policy and removes the legacy key', async () => {
    const projected = await projectEpoch2Config({ rawConfig: { networkProxyPolicy: { mode: 'system', proxyRules: 'ignored', credentialRef: 'ignored' } } })
