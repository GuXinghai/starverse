...
export function compileOllamaChatPreparedRequestV2(input:Readonly<{context:GenerationV2AuthorityTransactionContextV2;execution:GenerationExecutionOperationBundleV2;profile:LocalEndpointProfileV2;history:OllamaChatRequestHistoryFactV2}>):PreparedProviderRequestV2{
  ...
  const entries=[{kind:'consumed' as const,path:'reasoning.mode',disposition:'encoded' as const,nativeField:'think',evidence:'ollama-chat-v1'},
    {kind:'consumed' as const,path:'web.mode',disposition:'accepted_no_wire' as const,nativeField:null,evidence:'ollama-chat-v1'},
    {kind:'consumed' as const,path:'image.mode',disposition:'accepted_no_wire' as const,nativeField:null,evidence:'ollama-chat-v1'},
    {kind:'consumed' as const,path:'tools.mode',disposition:'accepted_no_wire' as const,nativeField:null,evidence:'ollama-chat-v1'},
    {kind:'consumed' as const,path:'providerExtension.kind',disposition:'accepted_no_wire' as const,nativeField:null,evidence:'ollama-chat-v1'},
    ...(i.reasoning.mode==='enabled'&&i.reasoning.effort?[{kind:'consumed' as const,path:'reasoning.effort',disposition:'encoded' as const,nativeField:'think',evidence:'ollama-chat-v1'}]:[]),
    ...([['maxOutputTokens','num_predict'],['temperature','temperature'],['topP','top_p'],['topK','top_k'],['seed','seed'],['stop','stop'],['repetitionPenalty','repeat_penalty']] as const).flatMap(([key,native])=>g[key]===undefined?[]:[{kind:'consumed' as const,path:`generation.${key}`,disposition:'encoded' as const,nativeField:`options.${native}`,evidence:'ollama-chat-v1'}])]
