...
export function commitLmStudioCurrentSnapshotV2(input: Readonly<{
  ...
  const commandBranchId = input.command.kind === 'lmstudio_plain_text_initial'
  ...
  if ((!initial && !regenerate && !edit) || !isGenerationCommandFactsAuthorityForContextV2(input.commandFacts, input.context) ||
      input.command.operationId.value !== input.pending.operationId.value || commandBranchId.value !== pendingSourceBranchIdV2(input.pending).value ||
