...
import { commitAnthropicPlainTextRetrySnapshotV2 } from './anthropicPlainTextSnapshotCommitV2'
import { loadGenerationSnapshotToolRegistryAuthorityV2 } from './generationToolRegistryAuthorityV2'

export class AnthropicPlainTextRetryCoordinatorV2Error extends Error {
  constructor(readonly code:
    | 'GENERATION_V2_ANTHROPIC_RETRY_TARGET_INVALID'
    | 'GENERATION_V2_ANTHROPIC_RETRY_CREDENTIAL_INVALID') {
    super(code)
    this.name = 'AnthropicPlainTextRetryCoordinatorV2Error'
  }
}

export function createAnthropicPlainTextRetryCoordinatorV2(input: Readonly<{
  ...
  return Object.freeze({
    submit: async (raw: unknown): Promise<GenerationTextCommandResultV2> => {
      const command = decodeAnthropicPlainTextRetryCommandV2(raw)
      const existing = replay(command)
      if (existing) return existing
      const credential = await input.credentialService.getStatus('anthropic')
      if (!credential.configured || !credential.credentialScopeId) {
        throw new AnthropicPlainTextRetryCoordinatorV2Error('GENERATION_V2_ANTHROPIC_RETRY_CREDENTIAL_INVALID')
      }
      try {
        return runGenerationV2AuthorityTransactionOnOwnedConnectionV2(input.db, (context) => {
          ...
          if (!targetRow || typeof targetRow.operationId !== 'string') {
            throw new AnthropicPlainTextRetryCoordinatorV2Error('GENERATION_V2_ANTHROPIC_RETRY_TARGET_INVALID')
          }
          ...
          if (!target || target.operation.resultAnswerRootId.value !== command.targetAnswerRootId.value ||
              ...
              target.snapshot.providerBinding.providerId.value !== 'anthropic' ||
          ...
          const persisted = commitAnthropicPlainTextRetrySnapshotV2({ context, executionRepo, pending, command, target })
