...
import { decodeAnthropicPlainTextEditResendCommandV2, type AnthropicPlainTextEditResendCommandV2 } from '../../src/next/generation-v2/providers/anthropic/plainTextEditResendCommandV2'
...
import { commitVerifiedAnthropicPlainTextEditResendSnapshotV2 } from './anthropicPlainTextSnapshotCommitV2'
...
export function createAnthropicPlainTextEditResendCoordinatorV2(input: Readonly<{
  ...
  function replay(command: AnthropicPlainTextEditResendCommandV2): GenerationTextCommandResultV2 | null {
    const observed = executionRepo.findOperation(command.operationId.value)
    if (!observed) return null
    if (observed.operation.actionKind !== 'edit_resend' || observed.operation.commandFingerprint !== command.requestFingerprint || observed.snapshot.providerBinding.providerId.value !== 'anthropic') {
    ...
    return runGenerationV2AuthorityTransactionOnOwnedConnectionV2(input.db, (context) => {
      ...
      if (!execution || execution.operation.actionKind !== 'edit_resend' || execution.operation.commandFingerprint !== command.requestFingerprint || execution.snapshot.providerBinding.providerId.value !== 'anthropic') {
        throw new GenerationExecutionV2RepoError('GENERATION_V2_EXECUTION_IDEMPOTENCY_CONFLICT')
      }
  ...
  return Object.freeze({
    submit: async (request: Readonly<{ command: unknown; expectedCredentialRevision: number; expectedCredentialScopeId: CredentialScopeIdV2; signal?: AbortSignal }>): Promise<GenerationTextCommandResultV2> => {
      const command = decodeAnthropicPlainTextEditResendCommandV2(request.command)
      ...
      try {
        ...
        return await modelEvidenceService.withRefreshedExactModelEvidence({
          ...
          consume: (modelEvidence) => runGenerationV2AuthorityTransactionOnOwnedConnectionV2(input.db, (context) => {
            ...
            if (raced) {
              if (raced.operation.actionKind !== 'edit_resend' || raced.operation.commandFingerprint !== command.requestFingerprint || raced.snapshot.providerBinding.providerId.value !== 'anthropic') {
                throw new GenerationExecutionV2RepoError('GENERATION_V2_EXECUTION_IDEMPOTENCY_CONFLICT')
              }
              const history = historyRepo.loadRequestHistory(context, command.operationId.value)
            ...
            const pending = graphRepo.beginEditedTurn(context, {
              operationId: command.operationId.value, mode: command.mode, branchId: command.branchId.value,
              sourceQuestionId: command.sourceQuestionId.value, sourceAnswerRootId: command.sourceAnswerRootId.value,
              expectedHeadMessageId: command.expectedHeadMessageId.value, questionId: createQuestionId(),
              answerRootId: createAnswerId(), userBody: command.userBody, createdAtMs: nowMs(),
            })
            return withSynchronousGenerationCommandFactsAuthorityV2(context, configRepo, attachmentRepo, pending.conversationId.value, command.commandAttachments, undefined, (commandFacts) => {
              ...
              return withVerifiedAnthropicGenerationAuthoritiesV2({ context, modelEvidence, commandFacts, toolRegistry, operation: 'text', use: ({ binding, capability }) => {
                const persisted = commitVerifiedAnthropicPlainTextEditResendSnapshotV2({ context, executionRepo, capabilityRepo, pending, command, commandFacts, binding, capability, toolRegistry, attachmentDescriptors })
