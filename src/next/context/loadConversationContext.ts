import { listMessages, type PersistedMessage } from '@/next/message/messageClient'
...
export function toInternalMessagesFromPersisted(rows: ReadonlyArray<PersistedMessage>): InternalMessage[] {
  ...
  for (const row of rows) {
    ...
    if (roleRaw === 'assistant' && isEmptyText && isEmptyBlocks && !hasMeaningfulMeta) {
      // Drop empty placeholders (e.g. crash/pending assistant) from request context.
      continue
    }
...
export async function loadConversationContextMessages(
  convoId: string,
  params?: Readonly<{ limit?: number }>
): Promise<InternalMessage[]> {
  const rows = await listMessages(convoId, { direction: 'asc', limit: params?.limit ?? 200 })
  return toInternalMessagesFromPersisted(rows)
}
