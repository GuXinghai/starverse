...
export type LocalEndpointTextChatOptions = Readonly<{
  requestId: string
...
type LocalEndpointTextChatBridge = Readonly<{
...
function getLocalEndpointTextChatBridge(): LocalEndpointTextChatBridge | null {
  const bridge = (globalThis as any).localEndpointChat as Partial<LocalEndpointTextChatBridge> | undefined
  ...
  return bridge as LocalEndpointTextChatBridge
}
...
export function buildLocalEndpointTextChatMessages(input: Readonly<{
  contextMessages?: readonly unknown[]
  userText: string
  currentUserContentBlocks?: ReadonlyArray<ProviderRuntimeContentBlock>
}>): LocalEndpointTextChatMessage[] {
  const messages: LocalEndpointTextChatMessage[] = []
...
async function* wireEventStream(input: Readonly<{
  bridge: LocalEndpointTextChatBridge
  requestId: string
    ...
    if (isStartFailure(startResult)) {
      ...
      queue.push({
        ...
        error: {
          ...
          code: String(startResult.code ?? 'local_endpoint_start_failed'),
          message: String(startResult.error ?? 'Local endpoint text chat failed to start.'),
        },
      })
      ended = true
      notify()
...
export async function* streamLocalEndpointTextChatAsDomainEvents(
  options: LocalEndpointTextChatOptions,
  ...
  const bridge = getLocalEndpointTextChatBridge()
  if (!bridge) {
    yield* semanticMapIpcMissingError(requestContext)
    return
  }

  const messages = buildLocalEndpointTextChatMessages({
  ...
  if (messages.length === 0) {
    yield* semanticMapIpcStartInvokeError(
      { ok: false, code: 'invalid_payload', error: 'Local endpoint text chat requires a text message.' },
      true,
      requestContext,
    )
    return
