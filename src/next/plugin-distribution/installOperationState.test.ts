...
describe('official install operation state machine', () => {
  ...
  it('accepts resumable download retry and pause transitions', () => {
    expect(validateOfficialInstallOperationTransition('downloading', 'retrying')).toEqual({ ok: true })
    expect(validateOfficialInstallOperationTransition('retrying', 'downloading')).toEqual({ ok: true })
    expect(validateOfficialInstallOperationTransition('retrying', 'paused_retryable')).toEqual({ ok: true })
  })
  ...
  it('rejects invalid and terminal transitions', () => {
    ...
    expect(validateOfficialInstallOperationTransition('paused_retryable', 'downloading')).toEqual({
      ok: false,
      reason: 'terminal_state',
    })
  })
  ...
  it('distinguishes active and terminal operation states', () => {
    ...
    expect(isOfficialInstallOperationActive('retrying')).toBe(true)
    expect(isOfficialInstallOperationActive('failed')).toBe(false)
    expect(isOfficialInstallOperationActive('paused_retryable')).toBe(false)
    expect(isOfficialInstallOperationTerminal('installed')).toBe(true)
    expect(isOfficialInstallOperationTerminal('paused_retryable')).toBe(true)
