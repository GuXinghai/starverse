...
export class DeepSeekPlainTextEditResendCommandV2Error extends Error {
  constructor(readonly code: 'GENERATION_V2_DEEPSEEK_EDIT_RESEND_COMMAND_INVALID') {
    super(code)
    this.name = 'DeepSeekPlainTextEditResendCommandV2Error'
  }
}

export type DeepSeekPlainTextEditResendCommandV2 = Readonly<{
  schemaVersion: 1
...
export function isDeepSeekPlainTextEditResendCommandV2(
  value: unknown,
): value is DeepSeekPlainTextEditResendCommandV2 {
  return Boolean(value && typeof value === 'object' && commands.has(value))
}

function invalid(): never {
  throw new DeepSeekPlainTextEditResendCommandV2Error('GENERATION_V2_DEEPSEEK_EDIT_RESEND_COMMAND_INVALID')
}

export function decodeDeepSeekPlainTextEditResendCommandV2(
  value: unknown,
  ...
  try {
    ...
    if (error instanceof DeepSeekPlainTextEditResendCommandV2Error) throw error
    throw new DeepSeekPlainTextEditResendCommandV2Error('GENERATION_V2_DEEPSEEK_EDIT_RESEND_COMMAND_INVALID')
  }
}
