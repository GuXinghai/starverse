...
describe('OpenAI-compatible baseline capability V2', () => {
  it('advertises only fixed codec fields and explicit reasoning mapping sources', () => {
    ...
    const capability = composeOpenAIChatCompatibleBaselineCapabilityV2({ binding, resolvedAt: '2026-07-20T00:00:00.000Z',
    ...
    expect(capability.fields.find((field) => field.path === 'reasoning.summary')).toMatchObject({ state: 'unavailable' })
