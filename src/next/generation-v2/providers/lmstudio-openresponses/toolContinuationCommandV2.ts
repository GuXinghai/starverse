...
export type LmStudioOpenResponsesToolOutputV2 = Readonly<{
  toolCallId: string
  content: string
  userConfirmedExternalSideEffect: boolean
}>

export type LmStudioOpenResponsesToolContinuationCommandV2 = Readonly<{
  ...
  toolOutputs: readonly LmStudioOpenResponsesToolOutputV2[]
  requestFingerprint: string
}>

export class LmStudioOpenResponsesToolContinuationCommandV2Error extends Error {
  constructor(readonly code: 'GENERATION_V2_LMSTUDIO_TOOL_CONTINUATION_COMMAND_INVALID') {
    super(code); this.name = 'LmStudioOpenResponsesToolContinuationCommandV2Error'
  }
}
...
function invalid(): never {
  throw new LmStudioOpenResponsesToolContinuationCommandV2Error(
    'GENERATION_V2_LMSTUDIO_TOOL_CONTINUATION_COMMAND_INVALID',
  )
}

export function decodeLmStudioOpenResponsesToolContinuationCommandV2(
  value: unknown,
): LmStudioOpenResponsesToolContinuationCommandV2 {
  ...
  const outputs = input.toolOutputs.map((raw) => {
    ...
    return Object.freeze({ toolCallId: output.toolCallId, content: output.content,
      userConfirmedExternalSideEffect: output.userConfirmedExternalSideEffect })
  })
  if (new Set(outputs.map((output) => output.toolCallId)).size !== outputs.length) invalid()
  try {
...
export function isLmStudioOpenResponsesToolContinuationCommandV2(
  value: unknown,
): value is LmStudioOpenResponsesToolContinuationCommandV2 {
