...
export function composeGenericLocalOpenAIChatBaselineCapabilityV2(input: Readonly<{ binding: DecodedProviderBindingRecordV2; resolvedAt: string }>): DecodedRuntimeCapabilitySnapshotV2 {
  ...
  return decodeRuntimeCapabilitySnapshotV2(canonicalizeUnverifiedRuntimeCapabilitySnapshotV2({ schemaVersion: 2,
    resolvedAt: input.resolvedAt, binding: projectDecodedProviderBindingRecordV2(input.binding), evidence: [{ evidenceId,
      kind: 'official_documentation', effect: 'supports', sourceRef: 'generic-local-explicit-openai-chat-profile-v1',
      verifiedAt: input.resolvedAt, contentDigest: GENERIC_LOCAL_OPENAI_CHAT_CONTRACT_DIGEST_V2 }], fields,
    tools: [], continuation: { kind: 'client_managed_native_replay', artifactKind: 'generic_local_openai_chat_messages',
