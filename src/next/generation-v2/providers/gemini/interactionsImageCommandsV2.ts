...
export function decodeGeminiInteractionsImageRegenerateCommandV2(value:unknown):GeminiInteractionsImageRegenerateCommandV2{
  try{const raw=object(value,['operationId','branchId','questionId','expectedHeadMessageId','modelId','commandAttachments']);const operationId=GenerationV2Identity.create('operation_id',text(raw.operationId));const branchId=ConversationGraphV2Identity.create('branch_id',text(raw.branchId));const questionId=ConversationGraphV2Identity.create('question_id',text(raw.questionId));const expectedHeadMessageId=ConversationGraphV2Identity.create('message_id',text(raw.expectedHeadMessageId));const modelId=model(raw.modelId);const commandAttachments=decodeGenerationCommandAttachmentsV2(raw.commandAttachments);const projection={schemaVersion:1 as const,kind:'gemini_interactions_image_regenerate' as const,operationId:operationId.value,branchId:branchId.value,questionId:questionId.value,expectedHeadMessageId:expectedHeadMessageId.value,modelId:modelId.value,commandAttachments:projectGenerationCommandAttachmentsV2(commandAttachments)};return command(projection,{...projection,operationId,branchId,questionId,expectedHeadMessageId,modelId,commandAttachments})}catch{throw new Error('GENERATION_V2_GEMINI_INTERACTIONS_IMAGE_REGENERATE_COMMAND_INVALID')}}
