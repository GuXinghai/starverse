...
export type OpenAIResponsesToolOutputV2 = Readonly<{
  toolCallId: string
  content: string
  userConfirmedExternalSideEffect: boolean
}>

export type OpenAIResponsesToolContinuationCommandV2 = Readonly<{
  ...
  toolOutputs: readonly OpenAIResponsesToolOutputV2[]
  requestFingerprint: string
}>

export class OpenAIResponsesToolContinuationCommandV2Error extends Error {
  constructor(readonly code: 'GENERATION_V2_OPENAI_TOOL_CONTINUATION_COMMAND_INVALID') {
    super(code)
    this.name = 'OpenAIResponsesToolContinuationCommandV2Error'
  }
}
...
function invalid(): never {
  throw new OpenAIResponsesToolContinuationCommandV2Error(
    'GENERATION_V2_OPENAI_TOOL_CONTINUATION_COMMAND_INVALID',
  )
}

export function decodeOpenAIResponsesToolContinuationCommandV2(value: unknown): OpenAIResponsesToolContinuationCommandV2 {
  ...
  try {
    ...
    const canonical = stableSerializeProviderRequestV2({
      ...
      expectedHeadMessageId: expectedHeadMessageId.value, priorRequestSequence: input.priorRequestSequence,
      toolOutputs: outputs,
    })
...
export function isOpenAIResponsesToolContinuationCommandV2(value: unknown): value is OpenAIResponsesToolContinuationCommandV2 {
