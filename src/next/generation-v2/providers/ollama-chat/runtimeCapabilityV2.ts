import { canonicalizeUnverifiedRuntimeCapabilitySnapshotV2, decodeRuntimeCapabilitySnapshotV2,
...
export function composeOllamaChatCapabilityV2(input: Readonly<{ binding: DecodedProviderBindingRecordV2; profile: LocalEndpointProfileV2; resolvedAt: string }>): DecodedRuntimeCapabilitySnapshotV2 {
  ...
  const fields: PersistedRuntimeCapabilityFieldV2[] = RUNTIME_CAPABILITY_SEMANTIC_PATHS_V2.map((path) => {
    ...
    return Object.freeze({path,state:'unavailable',constraints:Object.freeze([]),evidenceIds:Object.freeze([])}) as PersistedRuntimeCapabilityFieldV2
  })
  return decodeRuntimeCapabilitySnapshotV2(canonicalizeUnverifiedRuntimeCapabilitySnapshotV2({schemaVersion:2,resolvedAt:input.resolvedAt,
    binding:projectDecodedProviderBindingRecordV2(input.binding),evidence:[{evidenceId,kind:'user_narrowing_override',effect:'supports',
      sourceRef:`ollama-explicit-profile:${input.profile.endpointProfileId}`,verifiedAt:input.resolvedAt,contentDigest:input.profile.profileDigest}],fields,tools:[],
