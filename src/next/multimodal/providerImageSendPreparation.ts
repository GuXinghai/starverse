...
export type ProviderImageRuntimeProvider =
  | 'openai_responses'
  | 'anthropic_messages'
  | 'google_ai_studio'
  | 'openrouter'

export type PrepareProviderImageSendInput = Readonly<{
  provider: ProviderImageRuntimeProvider
...
export type PreparedProviderImageSend =
  | Readonly<{
      ok: true
      provider: ProviderImageRuntimeProvider
      sendPlan: SendPlan
...
export async function prepareProviderImageSendFromDraft(
  input: PrepareProviderImageSendInput
  ...
  let raw: unknown
  try {
    raw = await bridge.invoke('providerFileInput.prepareDraftImages', {
  ...
  if (record.ok === false) {
    return {
      ...
      message: String(record.message ?? 'Provider image input preparation failed.'),
...
function isMissingProviderFileInputBridgeError(error: unknown): boolean {
  ...
  return /providerFileInput\.prepareDraftImages|unknown method|not registered|unsupported method|method.*not/i.test(message)
}
