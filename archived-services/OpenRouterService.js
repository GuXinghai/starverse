...
import { extractTextFromMessage } from '../src/types/chat'
import { buildOpenRouterRequest } from '../src/services/providers/generationAdapter'
...
export const OpenRouterService = {
  ...
  async getModelParameters(apiKey, modelId, baseUrl = OPENROUTER_BASE_URL, provider = null) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // 404 是常见错误（模型不存在或不支持参数查询），降低日志级别
  ...
  let systemInstruction = null
    ...
    try {
      ...
      // 🔍 DEBUG: 发送请求前的详细信息
      console.log('OpenRouterService: 📤 即将发送 fetch 请求', {
  ...
  console.log('OpenRouterService: 正在发送请求到:', url)
      ...
      // 🔍 DEBUG: 收到 HTTP 响应
      console.log('OpenRouterService: ✅ 收到 HTTP 响应', {
        status: response.status,
        statusText: response.statusText,
        ok: response.ok,
        headers: {
          contentType: response.headers.get('content-type'),
          requestId: response.headers.get('x-request-id') || response.headers.get('x-openrouter-id')
        },
        timestamp: Date.now()
      })
      ...
      if (!response.ok) {
        ...
        // 🔧 新策略：通过 yield 返回错误信息，让调用方创建 role: 'openrouter' 消息
        const errorPayload = {
          type: 'openrouter_error',
          status: response.status,
          error: buildOpenRouterError(apiErrorInfo, response.status, `OpenRouter API 错误: ${response.status}`),
        ...
        if (response.status === 429) {
          ...
          errorPayload.error.message = `OpenRouter 速率限制：请求过于频繁，请等待 ${waitTime} 后重试`
          errorPayload.error.retryable = true
          if (retryAfter) {
            errorPayload.error.retryAfter = retryAfter
          }
        } else if (response.status === 401 || response.status === 403) {
          errorPayload.error.message = 'OpenRouter 认证失败：API Key 无效或已过期，请检查设置'
          errorPayload.error.retryable = false
        } else {
          if (!errorPayload.error.message || errorPayload.error.message === `OpenRouter API 错误: ${response.status}`) {
            errorPayload.error.message = `OpenRouter API 请求失败: ${response.status} - ${errorText}`
          }
          if (errorPayload.error.retryable === undefined) {
            errorPayload.error.retryable = response.status >= 500
          }
        }
        
        // ⚠️ yield 错误信息后直接返回，不抛出异常
        yield errorPayload
        return
      }
      ...
      console.log('OpenRouterService: 🔄 准备读取 response.body stream', {
  ...
  let requestId = requestIdFromHeader
      ...
      while (true) {
        ...
        if (done) {
          console.log('OpenRouterService: 🏁 流式读取完成 (done=true)')
          break
        }
        ...
        // 🔍 DEBUG: 接收到原始数据块
        console.log('OpenRouterService: 📦 收到数据块', {
        ...
        for (const line of lines) {
          ...
          if (trimmedLine.startsWith('data:')) {
            ...
            try {
              ...
              // 🔍 DEBUG: 解析到的 chunk 内容
              console.log('OpenRouterService: 🔍 解析 chunk', {
              ...
              if (!delta) {
                // 🔧 FIX: 即使没有 delta，也应该检查是否有其他有效数据
                // 避免完全静默导致超时定时器无法清除
                console.log('[OpenRouterService] ⚠️ chunk 没有 delta，但可能有其他数据', {
                  hasMessage: !!primaryChoice?.message,
                  hasFinishReason: !!primaryChoice?.finish_reason,
                  chunkKeys: Object.keys(chunk),
                  choiceKeys: primaryChoice ? Object.keys(primaryChoice) : []
                })
                
                // 如果有 message.content（非流式响应的完整消息），也要 yield
                if (primaryChoice?.message?.content) {
                  ...
                  if (typeof messageContent === 'string' && messageContent) {
                    console.log('[OpenRouterService] 📤 yield message.content (非流式格式):', messageContent.substring(0, 50))
                    yield { type: 'text', content: messageContent }
                    continue
                  }
                }
              ...
              if (Array.isArray(content)) {
                ...
                for (const block of content) {
                  ...
                  if (normalizedBlockImage) {
                    ...
                    console.log('OpenRouterService: 📤 yield image chunk')
                ...
                console.log('OpenRouterService: 📤 yield text chunk:', content.substring(0, 30))
                yield { type: 'text', content }
              } else if (content && typeof content === 'object') {
                // 如果 content 是对象
                if (content.text) {
                  console.log('OpenRouterService: 📤 yield text chunk (from object):', content.text.substring(0, 30))
