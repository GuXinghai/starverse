...
const createGenerationV2TestBridge = () => {
  ...
  workspace: {
    ...
    readBranch: vi.fn(async () => ok({ branchId, conversationId, projectId, title: 'New Chat', branchName: null,
      headMessageId: null, turns: [] })),
    listQuestionCandidates: vi.fn(async () => ok([])),
    selectQuestionCandidate: vi.fn(async () => ok({ headMessageId: null, chosenAnswerRootId: null })),
    selectAnswer: vi.fn(async () => ok({})),
