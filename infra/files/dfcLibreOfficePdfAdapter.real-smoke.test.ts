...
describeRealLibreOffice('DFC LibreOffice DOCX to PDF real managed runtime smoke', () => {
  it('runs managed soffice through the adapter and validates controlled PDF output', async () => {
    ...
    if (!availability.ok) throw new Error('LibreOffice managed runtime unavailable.')

    const sandboxRootDir = await mkdtemp(path.join(os.tmpdir(), 'starverse-dfc-office-pdf-real-smoke-'))
    const result = await runDfcLibreOfficeDocxToPdfAdapter({
      assetId: 'asset-docx-office-pdf-real-smoke',
    ...
    if (!result.ok) throw new Error('LibreOffice managed smoke failed.')
...
<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
  <w:body>
    <w:p><w:r><w:t>Starverse Office PDF dev smoke</w:t></w:r></w:p>
    <w:p><w:r><w:t>Managed LibreOffice runtime conversion.</w:t></w:r></w:p>
    <w:sectPr/>
  </w:body>
</w:document>`,
    },
