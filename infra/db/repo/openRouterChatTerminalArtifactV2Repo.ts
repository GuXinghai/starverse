...
import {
  isOpenRouterChatTerminalArtifactV1,
  serializeOpenRouterChatTerminalArtifactV1,
  type OpenRouterChatTerminalArtifactV1,
} from '../../../src/next/generation-v2/providers/openrouter/terminalArtifactV1'
...
export class OpenRouterChatTerminalArtifactV2RepoError extends Error {
  constructor(readonly code: 'GENERATION_V2_OPENROUTER_TERMINAL_STATE_INVALID' | 'GENERATION_V2_OPENROUTER_TERMINAL_CONFLICT') {
    super(code); this.name = 'OpenRouterChatTerminalArtifactV2RepoError'
  }
}
export class OpenRouterChatTerminalArtifactV2Repo {
  ...
  insertRequestTerminal(context: GenerationV2AuthorityTransactionContextV2,
    ...
    artifact: OpenRouterChatTerminalArtifactV1, createdAtMs: number): void {
    ...
    if (existing) {
      if (existing.operationId !== request.operationId || existing.codecVersion !== artifact.artifactCodecVersion ||
        ...
        throw new OpenRouterChatTerminalArtifactV2RepoError('GENERATION_V2_OPENROUTER_TERMINAL_CONFLICT')
      }
      return
    }
  ...
  private invalid(): never { throw new OpenRouterChatTerminalArtifactV2RepoError('GENERATION_V2_OPENROUTER_TERMINAL_STATE_INVALID') }
}
