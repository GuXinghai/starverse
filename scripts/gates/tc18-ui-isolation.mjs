...
function checkUiSpecifier(spec) {
  const legacyRoot = /^@\/(stores|services|components|composables|utils|types)(\/|$)/
  if (legacyRoot.test(spec)) {
    return 'ui must not import legacy surfaces (@/stores|@/services|@/components|@/composables|@/utils|@/types)'
  }

  const relativeLegacyRoot = /^\.\.(?:\/\.\.)*\/(stores|services|components|composables|utils|types)(\/|$)/
  if (relativeLegacyRoot.test(spec)) {
    return 'ui must not reach legacy surfaces via relative imports (../stores|../services|...)'
  }

  const archived = /^(?:\.{1,2}\/)+archived-(components|services)(\/|$)/
  if (archived.test(spec)) {
    return 'ui must not import archived legacy code (archived-components/archived-services)'
  }

  if (spec.includes('src/components') || spec.includes('src/stores') || spec.includes('src/services')) {
    return 'ui must not import legacy src/components|src/stores|src/services by path'
  }
...
function scanTarget(target) {
  ...
  for (const fileAbs of files) {
    ...
    for (const { spec, index } of extractImportSpecifiers(text)) {
      const reason = target.id === 'next' ? checkNextSpecifier(spec) : checkUiSpecifier(spec)
      if (!reason) continue
