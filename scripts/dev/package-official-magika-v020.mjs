...
async function main() {
  ...
  try {
    ...
    await writeNormalizedTextAsset(
      path.join(SPEC_ROOT, 'magika-pure-js-runtime.mjs'),
      path.join(ENGINE, 'runtime', 'magika-pure-js-runtime.mjs')
    )
...
async function writeManifests() {
  ...
  await writeNormalizedTextAsset(path.join(SPEC_ROOT, 'MAGIKA-LICENSE.md'), path.join(STAGE, 'licenses', 'MAGIKA.md'))
  await writeNormalizedTextAsset(path.join(SPEC_ROOT, 'ATTRIBUTION.md'), path.join(STAGE, 'attribution', 'ATTRIBUTION.md'))
...
async function writeNormalizedTextAsset(source, destination) {
