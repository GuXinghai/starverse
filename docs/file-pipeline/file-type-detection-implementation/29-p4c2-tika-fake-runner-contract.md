...
type TikaRunnerInput = Readonly<{
  ...
  metadataAllowlist: readonly string[] | null  // null = block all (capability disabled)
...
- `metadataAllowlist: null` → 阻止所有 metadata 输出（capability disabled 语义，同 `[]`）
- `metadataAllowlist: []` → 阻止所有 metadata 输出
