...
**Current-status note (2026-05-21)**: the bundled-with-app runtime option below is historical planning context, not the current production direction. Current Magika work uses the managed-plugin/runtime package model with explicit health verification.
...
**Historical P5-E planning note**: **Bundled with app** was considered as a default production path in this planning snapshot. It is no longer the current Magika direction; managed plugin packages with explicit verification/health checks supersede this recommendation.
...
| Option | Description | Pro | Con | Recommendation |
|--------|-------------|-----|-----|----------------|
| **A: Bundled with app** | `extraResources` in electron-builder; engines copied to userData on first launch | Always available, no network | Large installer, slow updates | Historical planning option; not current Magika direction |
