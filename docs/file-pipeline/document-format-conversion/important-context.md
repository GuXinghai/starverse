...
- Latest closeout / evidence bundle: `dfc-goal-v1.2-closure-living-plan.md` and `dfc-goal-v1.2-closure-progress.md` record the current DFC v1.2 phase-closure baseline. The supporting evidence anchors are M63 (`dfc-m63-libreoffice-plugin-availability-smoke-and-production-closeout.md`), M64 attachment productization (`dfc-goal-attachment-productization-living-plan.md`, `dfc-goal-attachment-productization-progress.md`, and the `DFC-M64` / `DFC-M64-P2` rows in `progress-ledger.md`), plus the supported-matrix addenda in `dfc-m1-phase-1-closeout-supported-matrix.md`.
- Current recovery state: DFC v1.2 is stage-closed around text, markdown, code, CSV/TSV, HTML markdown/code/pdf_attachment, XLSX table_markdown pilot, DOCX markdown pilot, and Windows x64 DOCX pdf_attachment. The current documentation-only closeout patch passed targeted DFC/UI/contract/i18n/Send Plan tests, targeted worker format/unsupported matrix tests, `vue-tsc`, `git diff --check`, and the privacy gate. LibreOffice DOCX-to-PDF is production-approved only for the validated Windows x64 managed `.svpkg` path. The scoped approval is `productionApproved=true`, `approvedPlatform=win32`, `approvedArch=x64`, `approvedInput=docx`, `approvedOutput=pdf_attachment`, approved acquisition modes `manual_github_release` and `offline_import`, with automatic/startup/background/postinstall/conversion-time download still disabled. macOS and Linux packages are deferred, not rejected, and require independent platform package preparation, trust/catalog entries, packaged smoke, PDF validation, and legal/provenance updates. No LibreOffice binary, `.svpkg`, extracted runtime, system LibreOffice/PATH fallback, `.doc` / `.rtf` / `.docm`, `.xls`, Excel-to-PDF, PS/EPS, PDF OCR/local parsing, image/audio support, arbitrary executable paths, renderer-provided executable paths, arbitrary plugin URLs, external engine, new dependency, or automatic download behavior is committed or approved.
- Closure interpretation note: wider legacy/future candidate families in file classification and route mapping are not implementation support claims. The v1.2 support authority is the execution layer, backend-owned DFC options, Send Plan validation, managed runtime gates, and the supported-matrix closure addendum.
...
- Next DFC work should not reopen v1.2 closure unless an owner decision expands the support matrix. Do not expand Office formats, approve macOS/Linux packages, enable automatic/startup/background/postinstall/conversion-time downloads, add system/PATH fallback, allow arbitrary executable paths, allow arbitrary package URLs, add external engines/dependencies, or loosen sandbox/path-cap/diagnostics privacy boundaries unless Owner explicitly approves that separate scope.
...
## 2026-06-25 DFC v1.2 closure baseline notes

- DFC v1.2 closure is intentionally lightweight and documentation-first: no new format support, runtime, dependency, DB schema, Send Plan semantic change, download policy, or UI architecture is introduced.
- The stage-closed implemented scope is text, markdown, code, CSV/TSV, HTML markdown/code/pdf_attachment, XLSX table_markdown pilot, DOCX markdown pilot, and Windows x64 DOCX pdf_attachment.
- Deferred or explicitly unsupported items remain `.doc`, `.rtf`, `.docm`, `.xls`, Excel-to-PDF, PS/EPS, PDF OCR/local parsing, macOS/Linux LibreOffice, system LibreOffice, PATH fallback, automatic download, external engines, and new dependencies.
- The broader `starverse_format_conversion_preview_v1_2.md` design document remains a product/architecture candidate spec; the current implementation baseline is governed by the supported matrix addenda, progress ledger, and closure living plan.
- `public/build-id.json` remains dirty as a generated build-id artifact and must not be committed with DFC closure docs.
