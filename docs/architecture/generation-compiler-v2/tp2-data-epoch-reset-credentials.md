...
7. Fresh DB is created from V2 schema only; no legacy ensure/migration helpers run. Seed only system rows and `app_meta_v2.data_epoch=2`.
8. Epoch failure prevents generation IPC/window startup. Restart resumes the journal idempotently.
9. The epoch scope key is generated as 32 CSPRNG bytes and represented by canonical Base64 only for Electron main-process async `safeStorage`. The resulting JavaScript plaintext string cannot be reliably erased before garbage collection; this is an accepted Electron API boundary. It is never logged, included in an exception, cached, sent through IPC, or exposed to a renderer, and every controllable Buffer is zeroed immediately after use. Unavailable/temporarily unavailable storage, noncanonical Base64, a decoded length other than 32 bytes, or decryption failure fails closed. A valid decrypt with `shouldReEncrypt=true` performs a separate bootstrap-owned `BEGIN IMMEDIATE` compare-and-swap rewrap that may change only ciphertext, monotonically increment `envelope_revision`, and advance `updated_at_ms`; rewrap failure or a second immediate rewrap request fails closed without accepting a partially updated envelope.
10. On Windows, `safeStorage` uses the current user's DPAPI boundary. The accepted threat model protects persisted ciphertext from direct decryption by a different Windows user; it does not claim protection against malicious software running as the same user or against process-memory inspection.
11. `initializeOrVerifyFreshEpoch2Database()` is the bootstrap coordinator's internal and sole initialization-transaction boundary: it owns one `BEGIN IMMEDIATE` that includes every schema DDL statement, the schema manifest, `app_meta_v2`, and the initial `epoch_scope_key_envelope_v2` row. The outer phase/startup coordinator invokes this boundary but must not open or nest a database transaction around it. Transaction-prohibited PRAGMAs are configured and their returned values verified first. After commit, a new readonly connection must validate the exact closed SQLite object set, complete schema/object digest, canonical identity/epoch/root and decryptable envelope before activation. Recovery may leave a zero-byte or nonzero main file, rollback journal or WAL, but its accepted logical state is only empty/uninitialized or complete; schema-only identity gaps and unknown tables/views/indexes/triggers are corruption and are never patched.
