...
4. Freeze OpenAI/LM Studio continuation modes, OpenRouter beta-tool exposure, transport retry policy, and then implement the already-fixed reset and endpoint-descriptor policies.
...
2. Capability evidence/revision/UI projection matrix.
3. Snapshot/continuation/attachment artifact integrity.
4. Exact provider-native serialized request fixtures.
5. End-to-end semantic→capability→codec→bytes composition.
...
12. Representative live provider/protocol smoke plus packaged fresh-profile smoke.
...
Baseline IDs remain stable; AC-18 and AC-19 are corrected by current official evidence.
...
| AC-24 | One selected endpoint descriptor supports the complete image intent, limits UI/compiler, is pinned by its `provider_tag`, and obeys the configured refresh/hard-expiry policy without silent downgrade or endpoint fallback. |
