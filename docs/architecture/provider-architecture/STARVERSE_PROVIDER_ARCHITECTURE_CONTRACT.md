...
| 当前事实 | 证据来源 |
|---|---|
| 当前 active runtime 是 OpenRouter-first。主要链路是 `src/ui-app/app/appChatApp.logic.ts` -> `streamOpenRouterChatAsEvents` -> `buildOpenRouterChatCompletionsRequest` -> renderer fetch 或 `openrouter:*` IPC -> `decodeOpenRouterSSE` -> `mapChunkToEvents` -> state/repo/SQLite。 | `provider-architecture-current-state-integrated.md` §1, §3, §4；源码路径：`src/ui-app/app/appChatApp.logic.ts`、`src/ui-app/app/useLiveStreamController.ts`（原 `src/next/live/openRouterLiveStream.ts` 已移除，2026-08-14 修正）、`src/next/openrouter/buildRequest.ts`、`src/next/openrouter/sse/decoder.ts`、`src/next/openrouter/mapChunkToEvents.ts`。 |
...
| Gemini legacy identifiers remain `runtime-dead remnants` (`geminiApiKey`, `PROVIDERS.GEMINI`, legacy `activeProvider: 'Gemini'`); the retired legacy SDK dependency is removed and the active V2 path uses native REST. | `provider-architecture-current-state-integrated.md` §1, §6；`provider-architecture-source-comparison.md` §6, §7；源码/片段：`electron/config/configSchema.ts`、（原 `src/constants/providers.ts` 已移除，2026-08-14 修正）、`current-state-original/snippets/gemini-status.md`。 |
| Reasoning 已与 visible assistant text 分离，state/DB/reasoning segment/final JSON 有基础；但当前一等解析格式主要是 OpenRouter `reasoning_details`，没有 DeepSeek `reasoning_content`、Anthropic thinking、OpenAI Responses reasoning items、Gemini thought/thinking-like data 的一等 runtime parser。 | `provider-architecture-current-state-integrated.md` §1, §5, §8, §11；源码路径：`src/next/openrouter/mapChunkToEvents.ts`、`src/next/state/types.ts`、`src/next/state/reducers/reasoningHandlers.ts`、`infra/db/v2/*.sql`（原 `infra/db/schema.sql` 已移除，2026-08-14 修正）、`infra/db/repo/reasoningDetailsAggregator.ts`。 |
| Send Plan / catalog 有 provider-aware 或 neutral-ish 基础，但 UI、catalog query、web search、request builder、IPC、error envelope 仍明显 OpenRouter-coupled。 | `provider-architecture-current-state-integrated.md` §3, §5, §9, §10, §11, §15；源码路径：`src/shared/files/sendPlanTypes.ts`、`src/next/files/sendPlanClient.ts`（原 `infra/files/sendPlanService.ts` 已移除，2026-08-14 修正）、`src/next/openrouter/openRouterSendPlanSerializer.ts`、`src/next/openrouter/searchSettingsResolver.ts`、`electron/preload.ts`、`src/next/errors/openRouterErrorEnvelope.ts`、`src/ui-app/components/ModelPickerDialog.vue`。 |
...
| durable `usage_log` table/repo 存在，但 active send path 的 durable usage logging 未由证据确认；active send 证据显示 `UsageDelta` 进入 message metadata snapshot。 | `provider-architecture-current-state-integrated.md` §1, §5, §12, §14；`provider-architecture-source-comparison.md` §6, §7；源码路径：`infra/db/v2/*.sql`（原 `infra/db/schema.sql` 已移除，2026-08-14 修正）、`infra/db/repo/usageRepo.ts`、`src/ui-app/app/appChatApp.logic.ts`。 |
