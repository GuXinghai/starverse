...
| Gemini legacy identifiers remain `runtime-dead remnants` (`geminiApiKey`, `PROVIDERS.GEMINI`, legacy `activeProvider: 'Gemini'`); the retired legacy SDK dependency is removed and the active V2 path uses native REST. | `provider-architecture-current-state-integrated.md` §1, §6；`provider-architecture-source-comparison.md` §6, §7；源码/片段：`electron/config/configSchema.ts`、`src/constants/providers.ts`、`current-state-original/snippets/gemini-status.md`。 |
