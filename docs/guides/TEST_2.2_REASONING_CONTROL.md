...
### **Test Case 3: Reasoning disclosure + panelState（替代旧二态命名）**
...
#### 测试步骤
1. 打开推理面板（Reasoning Panel）
2. 验证 `visibility` 三态文案：`shown | excluded | not_returned`
3. 切换折叠/展开（panelState），验证仅影响展示，不得改变 `visibility`

#### 预期结果
- UI 能明确区分 `excluded` 与 `not_returned`：
  - `excluded`：本次请求明确要求不返回推理内容（对应 OpenRouter `reasoning.exclude=true`）。
  - `not_returned`：模型未返回推理内容 / 或该模型不支持（不得用“缺字段”推断 encrypted）。
- 折叠/展开只影响 `panelState`，不得改变 `visibility` 判定。

#### 验证点
- [ ] `visibility` 三态文案互斥且可解释
- [ ] `panelState` 切换不影响 `visibility`
...
1. 在对话 A 中：
   - 启用推理模式
   - 设置 Effort 为 "High"
   - 设置 disclosure 为 "Excluded"（即 `reasoning.exclude=true`）
2. 切换到对话 B
3. 在对话 B 中保持默认配置
4. 切换回对话 A
...
#### 前置条件
1. 启用推理模式
2. 配置特定 effort 与 disclosure（可选 excluded）
3. 打开浏览器开发者工具（Network 面板）

#### 测试步骤
1. 配置推理模式：
   - Effort: "High"
   - Disclosure: "Shown"（模型返回可展示推理时）
2. 发送一条消息
