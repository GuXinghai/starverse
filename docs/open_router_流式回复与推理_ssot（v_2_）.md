...
- 本仓库统一用 `reasoning.effort` 表达推理强度（不使用 `enabled` 快捷方式）。
- OpenRouter `reasoning.effort` 完整枚举：`none|minimal|low|medium|high|xhigh`。
- `reasoning.exclude=true` 是“允许内部推理但不返回推理 tokens”的开关（不是挡位）。
- `effort` 与 `max_tokens` 二选一（不得同时发送）。
- 请求体契约（与 reducer 记录的 requestedReasoning* 一致）：
  - **auto/omit**：省略 `reasoning` 字段（完全不发送）
  - **effort**：`reasoning: { effort: "<enum>", exclude?: boolean }`
  - 禁止：在本仓库请求体中使用 `enabled:true`；默认启用用 `effort:"medium"` 表达。

#### 2.1.1 产品默认（UI 默认值）
- **协议层默认（auto/omit）**：若未指定 `reasoning`，可以省略该字段（完全不发送）。
- **产品层默认（UI）**：默认选择 `effort=none`，即发送 `reasoning: { effort: "none" }`（显式禁用推理，避免默认成本与模型差异）。
- **Auto（omit）**：UI 必须提供该选项，但默认不选；用于“完全不发送 reasoning 字段”。
- **启用推理（推荐默认档位）**：用户从 `none` 切到非 `none` 时，推荐默认档位为 **medium**（UI 文案提示即可）。
- **exclude 默认**：`false`；仅当用户显式勾选时才发送 `exclude:true`（且 mode=auto 时必须为 false）。
...
### 2.6 Provider Routing 参数（provider.*）
- 本仓库允许注入 OpenRouter 的 **provider routing 参数**到请求体的 `provider` 对象中（HTTP body 使用 snake_case）。
- `provider.require_parameters`（默认 `false`）：
  - `false`：不支持某些参数的 provider 仍可能接到请求并忽略未知参数。
  - `true`：仅路由到支持**全部请求参数**的 provider（更严格，避免“参数被悄悄忽略”）。
- 约定（当前实现）：
  - 设置项：`openrouter.provider.require_parameters`（持久化于 SQLite settings KV）。
  - 请求体字段：当 setting=true 时注入 `provider: { require_parameters: true }`；当 setting=false 时省略（等价于 OpenRouter 默认行为）。
...
## 8. 模型目录同步（SQLite 扩展 SSOT，供 UI 选择列表使用）

> 本节只描述“目录/索引”的 SSOT 约束：**OpenRouter JSON 不进入 UI**；UI 只读 SQLite 派生结果（Facade/Hook + selectors）。

### 8.1 model_catalog（全字段覆写 + snapshot + 软隐藏）
- 来源：`GET /api/v1/models`（OpenRouter 官方模型目录；能力字段来自 `supported_parameters`）。
- 写入规则（单事务）：
  - 本次出现的 `model_id`：全字段覆写，并设置 `last_seen_snapshot_id = snapshot_id`、`is_hidden = 0`
  - 本次未出现的 `model_id`：仅标记 `is_hidden = 1`（不删除）
  - 同步失败：必须回滚，禁止产生半同步导致误隐藏

### 8.2 reasoning_model_index（从 catalog 派生的最小索引）
- 输入：SQLite `model_catalog`（以 catalog 中“最近成功 snapshot”作为同步标记来源）
- 判定：`supported_parameters` 包含 `'reasoning'`
- 同构同步规则：
  - 仅写入/覆写：`model_id`、`name`、`status`、`last_synced_snapshot`
  - `model_catalog.is_hidden=1` 的 reasoning 模型仍进入 index，且 `status='hidden'`
  - index 中存在但本次不再属于 reasoning 集合：置 `status='hidden'`（不删除）
