...
- Slices 1–3.5 are complete: the isomorphic Pack/Rule core, revisioned authoritative exact-subject-set service, revision-bound exact-claim materializer, active-source promotion, and runtime exact-claim consumer cutover are implemented. Committed subject changes await serialized rematerialization and surface publication failure before reporting usable success; runtime performs no selector or regex matching. The subject service accepts only current first-party active/LKG catalogs, active compatible remote/manual IDs, and explicit local profile bindings; forbidden metadata sources cannot create subjects.
...
- Slice 4 Cloud candidate acquisition is next. Goal 3 priority/merge/winner/conflict/final `capabilityRevision` remains deferred, and the removed bundled/query-bound/request-time Rules authority must not return as a fallback.
