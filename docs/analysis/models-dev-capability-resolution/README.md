...
- Slices 1–2 are complete: the isomorphic owner-neutral Pack/Rule core and the revisioned authoritative exact-subject-set service are implemented but intentionally dormant. The subject service accepts only current first-party active/LKG catalogs, active compatible remote/manual IDs, and explicit local profile bindings; forbidden metadata sources cannot create subjects. Slice 3 revision-bound materialization is next, and no parallel Rules authority has been published.
