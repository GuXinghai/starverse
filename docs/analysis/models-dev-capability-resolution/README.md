...
- Slices 1–4 are complete: the isomorphic Pack/Rule core, revisioned authoritative exact-subject-set service, revision-bound exact-claim materializer, active-source promotion, runtime exact-claim consumer cutover, and fixed-authority Cloud candidate acquisition are implemented. Committed subject changes await serialized rematerialization and surface publication failure before reporting usable success; runtime performs no selector or regex matching. The subject service accepts only current first-party active/LKG catalogs, active compatible remote/manual IDs, and explicit local profile bindings; forbidden metadata sources cannot create subjects. Cloud acquisition persists validated candidates and permanent version/content bindings but does not Apply them or create another Model Facts source.
...
- Slice 5 Cloud Apply/LKG/rollback/pin is next. Goal 3 priority/merge/winner/conflict/final `capabilityRevision` remains deferred, and the removed bundled/query-bound/request-time Rules authority must not return as a fallback.
