...
- UI implementation remains the next item 14 slice: Settings integration, Cloud/User Rules workflows, UI-safe services/IPC, Facts Inspector, and Model Picker deep-link. Goal 3 priority/merge/winner/conflict/final `capabilityRevision` remains deferred, and the removed bundled/query-bound/request-time Rules authority must not return as a fallback.
