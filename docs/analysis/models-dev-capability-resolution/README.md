...
- Slices 1–3 are complete and intentionally dormant: the isomorphic Pack/Rule core, revisioned authoritative exact-subject-set service, and revision-bound exact-claim materializer are implemented. Materialization persists a complete canonical Rule source behind a non-active stage pointer; it does not update the active source pointer or any runtime consumer. The subject service accepts only current first-party active/LKG catalogs, active compatible remote/manual IDs, and explicit local profile bindings; forbidden metadata sources cannot create subjects. Slice 3.5 authority cutover is next, and no parallel Rules authority has been activated.
