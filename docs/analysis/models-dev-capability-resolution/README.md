...
14. [Cloud-managed Capability Rules implementation Goal](14-cloud-managed-capability-rules-implementation-goal.md) — completed production implementation sequence for the shared Pack/Rule core, exact-subject materialization, explicit Rules authority cutover, Cloud/User lifecycle, and UI-safe surfaces; it does not itself authorize Goal 3, which was subsequently completed under item 15.
15. [Goal 3 three-source Model Facts resolution implementation plan](15-goal-3-three-source-resolution-implementation-plan.md) — current-code implementation map, seven dependency Slices, atomic consumer cutover closure, delete-after-migration inventory, validation gates, deferred ledger, and final closeout record; Goal 3 implementation and final bounded correction are complete under this plan.
16. [Original Word compilation](source/starverse-models-dev-capability-assessments.docx) — renamed, unmodified source evidence.
...
- Item 12 is the controlling post-Goal 2C UI/product amendment. It supersedes items 8–11 only for bundled built-in lifecycle, Cloud/User ownership semantics, Capability Rules source convergence, authoritative-subject identity input, and regex materialization timing. It does not replace the Goal 2B ontology or authorize Goal 3; Goal 3 was subsequently authorized under item 15.
...
- Item 14 is the completed production implementation Goal authorized after the read-only pre-implementation audit. It translated items 06/12/13 into dependency slices and added an explicit Slice 3.5 Rules authority cutover; it does not supersede those Owner contracts or itself authorize Goal 3.
- Item 15 is the controlling Goal 3 implementation plan at baseline `e928b5cc`. It maps the frozen contract to current production code and fixes the Slice, cutover, deletion, validation, and deferred boundaries; Goal 3 implementation and final bounded correction are complete under it.
- Current implementation status and remaining TODOs are split between items 7–15: item 7 records the authority-boundary residuals, item 8 records the historical Goal 2A migration, item 9 records the historical Goal 2A-Fix dataset/fact-quality closeout, item 10 is the frozen Goal 2B design, item 11 is the Goal 2C ingestion closeout, item 12 freezes the UI/product contract, item 13 freezes its Cloud distribution dependency, item 14 records the completed Capability Rules production migration, and item 15 controls Goal 3 implementation sequencing. Do not revive the deleted Catalog capability resolver while implementing Goal 3.
- The 2026-09-22 Goal 3 contract extraction is closed. Provider Native/models.dev stale current/LKG facts remain eligible at their configured source priority; stale is freshness/diagnostics only, causes neither automatic demotion nor exclusion, and freshness-only changes do not produce a new final `capabilityRevision`. Goal 3 Owner contract, implementation, and final bounded correction are closed under item 15.

## Current post-Goal 2C implementation status / TODO

- Production no longer contains the Goal 2C-era bundled/query-bound Rules authority. Item 14 records the completed Slice 3.5 cutover and controls the remaining Cloud/User/UI slices.
...
- Slices 1–5 are complete: the isomorphic Pack/Rule core, revisioned authoritative exact-subject-set service, revision-bound exact-claim materializer, active-source promotion, runtime exact-claim consumer cutover, fixed-authority Cloud candidate acquisition, and atomic Cloud Apply/LKG/history/rollback/pin lifecycle are implemented. Committed subject changes await serialized rematerialization and surface publication failure before reporting usable success; runtime performs no selector or regex matching. The subject service accepts only current first-party active/LKG catalogs, active compatible remote/manual IDs, and explicit local profile bindings; forbidden metadata sources cannot create subjects. Cloud Apply publishes through the one existing Capability Rules source and does not create another Model Facts source.
...
- The temporary Rules-only consumer projection was the pre-Goal-3 compatibility path; Goal 3 migration removed it. It is not a compatibility authority or a cross-source resolver.
- Slice 6 is complete: committed User Packs/Rules, full-snapshot durable draft/recovery, atomic Save/Cancel, shared Rewrite, typed whole-Pack Import/Export, and the ordinary first-Pack lifecycle are implemented without creating a second Rules source. Notes are deliberately non-fact metadata and Note-only Save does not churn the canonical Rules source revision.
- Slices 1–7 are complete, including Settings integration, Cloud/User Rules workflows, UI-safe services/IPC, Facts Inspector, exact Model Picker deep-links, and modal focus behavior. Goal 3 implementation and final bounded correction are complete under item 15. The removed bundled/query-bound/request-time Rules authority must not return as a fallback.
- Item 15 records the completed Goal 3 implementation and final bounded closeout. The plan requires direct all-claims Rules input, a separate source-priority revision, freshness-free semantic resolution, one atomic new-snapshot consumer cutover, frozen retry/replay continuity, and deletion of temporary/legacy authorities after migration.
