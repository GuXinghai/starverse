...
- [ ] 临时开关（generation pipeline switch）已删除：`src/next/config/flags.ts` 不存在，且 `useNextGenerationPipeline` / `readGenerationFlags` 在 `src/` 内无引用。
- [ ] legacy pipeline stub 已删除：`src/next/generation/legacyGenerationPipeline.ts` 不存在。
